...
async def chat(
    ...
    """
    # 1. 确定对话 ID
    # 所有同步 DB 操作通过 asyncio.to_thread 卸载到线程池，避免阻塞事件循环导致并发请求被串行化
    conversation_id = request.conversation_id
    if not conversation_id:
        auto_title = request.query[:20] + "..." if len(request.query) > 20 else request.query
        db_conv = await asyncio.to_thread(
            lambda: chat_crud.create_conversation(
                db,
        ...
        db_conv = await asyncio.to_thread(
            lambda: chat_crud.get_conversation(db, conversation_id=conversation_id)
        )
        if not db_conv or db_conv.user_id != current_user.id:
            raise HTTPException(status_code=403, detail="Not authorized to access this conversation")

    # 2. 记录用户提问
    db_user_msg = await asyncio.to_thread(
        lambda: chat_crud.create_message(
            db,
    ...
    # StreamingResponse 在路由函数返回后才开始消费生成器，此时 Depends(get_db)
    ...
    async def event_generator():
        ...
        try:
            try:
                async for chunk in dify_client.send_chat_message(
                    ...
                    if current_msg_id and not dify_message_id:
                        ...
                        try:
                            await asyncio.to_thread(
                                lambda: chat_crud.update_message_dify_id(stream_db, user_msg_id, dify_message_id)
                            )
                        except Exception:
                            await asyncio.to_thread(stream_db.rollback)
                    ...
                    if event in ("message", "agent_message"):
                        ...
                        try:
                            ...
                            await asyncio.to_thread(_update_membership)
                        except Exception:
                            await asyncio.to_thread(stream_db.rollback)
                        ...
                        try:
                            if new_dify_conv_id and not dify_conv_id:
                                await asyncio.to_thread(
                                    lambda: chat_crud.update_conversation_dify_id(stream_db, conversation_id, new_dify_conv_id)
                                )
                        except Exception:
                            await asyncio.to_thread(stream_db.rollback)
                ...
                yield f"data: {json.dumps({'event': 'error', 'message': 'AI 服务暂时不可用，请稍后重试'})}\n\n"
        finally:
            await asyncio.to_thread(stream_db.close)
